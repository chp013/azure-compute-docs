...
>[!NOTE]
>The host reserves a portion of the "Available vCPUs" and "Available RAM" listed in the tables below for its own operations. To determine the actual usable resources, please refer to the host [pricing page](https://aka.ms/ADHPricing).
>"Available vCPUs" does not account for VM overhead, which varies by VM size. This overhead consumes part of the vCPU allocation, meaning not all advertised vCPUs are available for use.
