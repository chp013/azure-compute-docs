...
The VM Agent is required to manage, install and execute extensions. If the VM Agent isn't running or is failing to report a Ready status to the Azure platform, then the extensions isn't working correctly.
...
/var/log/azure/
```
And the Extension settings and status files by detauls is in 
```
/var/lib/waagent/
```
