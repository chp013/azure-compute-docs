...
> [!NOTE]
> There is a known resize operation error that occurs when migrating from the NVv4-series to the NVadsV710_v5-series. Microsoft is working on a fix that will be implemented April 2026. In the meantime, we suggest that you follow this [workaround](/azure/virtual-machines/azure-vms-no-temp-disk#how-do-i-migrate-from-a-vm-size-with-local-temp-disk-to-a-vm-size-with-no-local-temp-disk---).
